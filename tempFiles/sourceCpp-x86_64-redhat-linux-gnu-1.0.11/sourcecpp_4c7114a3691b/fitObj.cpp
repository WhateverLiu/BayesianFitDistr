...
)
{
  ...
  // Rcout << "sequentialUpdate = " << sequentialUpdate << "\n";
...
)
{
  ...
  if (RIBlib == "Boost") return LBFGSBtrbFitList<0> (
    ...
    maxCore, sequentialUpdate, hgrad, centralDiff, max_iterations, m, epsilon, 
    epsilon_rel, past, delta, max_submin, max_linesearch, min_step, 
    max_step, ftol, wolfe);
