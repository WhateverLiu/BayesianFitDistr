...
if (T)
{
  ...
  png('../figure/mvAvgsDifferengWindowSizes.png', width = 8, height = 8,
      res = 120, units = "in")
  ...
  png('../figure/mvAvgsDifferengWindowSizes-dots.png', width = 8, height = 8,
      res = 120, units = "in")
  par(mar = c(4, 5, 0, 0), family = "serif", mfrow = c(4, 2))
  wsize = c(125, 300, 1000, 3000)
  for (i in 1:length(mvAvgs))
  {
    if (i == 1) ylab = "CDR" else ylab = ""
    if (i == 4) xlab = "MDR (mean in window)" else xlab = ""
    plot(x = mvAvgs[[i]]$mdr, y = mvAvgs[[i]]$cdr, type = "p", 
         col = scales::alpha('red', 0.5), 
         cex.lab = 2, cex.axis = 1.5, bty = "L", ylab = ylab, 
         xlim = mdrRange, ylim = cdrRange, xlab = xlab, las = 1, cex = 0.5, pch = 19)
    
    
    if (i == 1) legend(
      "bottomright", legend = c(expression("Mean"~"CDR"~"in"~"window"), expression(P[0])),
      cex = 1.8, bty = "n", col = scales::alpha(c('red', 'blue'), 0.5), 
      pch = c(19, 19))
    
    
    if (i == 1) ylab = expression(P[0]) else ylab = ""
    plot(x = mvAvgs[[i]]$mdr, y = mvAvgs[[i]]$p0, type = "p", 
         col = scales::alpha('blue', 0.5), 
         cex.lab = 2, cex.axis = 1.5, bty = "L", ylab = ylab, 
         xlim = mdrRange, ylim = p0Range, xlab = "", las = 1, cex = 0.5, pch = 19)
