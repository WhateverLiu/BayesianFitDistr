...
# ==============================================================================
windowSizeRatio = 0.001121089 # 200 data points in window.
slidingSpeedRatioAsWindowSize = 0.001121089 * 0.1
maxQuantile = 0.99
maxLowerBound = 0.05
...
if (T)
{
  ...
  if (F)
  {
    ...
    png("../figure/p0modelsFigure-windowSize-200.png", width = 10, height = 0.618 * 10, 
        unit = "in", res = 120)
...
if (T)
{
  ...
  if (F)
  {
    ...
    #     unit = "in", res = 120)
    png("../figure/TrBtransitionParam-windowSize-200.png", width = 9, 
        height = 9 * 0.618, unit = "in", res = 120)
...
if (T)
{
  ...
  pmftable = pmftablelist$tuned$p64
  pmftableBT = pmftablelist$untuned$p64
  ...
  if (F)
  {
    ...
    if (T)
    {
      ...
      #     height = 10 * 0.618)
      png("../figure/rawDiscretization-p0p1maxCv-MDR-windowSize-200.png",
          width = 10, height = 10 * 0.618, res = 120, unit = "in")
      par(mar = c(4.2, 5, 0, 0), family = "serif")
    ...
    if (T)
    {
      png("../figure/tunedDiscretization-p0p1maxCv-MDR-windowSize-200.png", 
          width = 10, height = 10 * 0.618, unit = "in", res = 120)
      par(mar = c(4.2, 5, 0, 0), family = "serif")
      layout(rbind(c(1, 1, 2, 2, 3, 3),
                   c(4, 5, 5, 6, 6, 7)))
    ...
    if (T)
    {
      ...
      pdf("../figure/pmfTable-45-windowSize-200.pdf", width = 10, height = 10 * 0.618)
    ...
    if (T)
    {
      ...
      whichPMFinTable2plot = match(tmp, as.integer(round(pmftable[1,] * 1e5)))
      tablePmf2plot = pmftableToDistrList(pmftable[, whichPMFinTable2plot, drop = F])
      pdf("../figure/pmfTable-207-windowSize-200.pdf", width = 10, height = 10 * 0.618)
      par(mar = c(4.2, 5, 0, 0), mfrow = c(2, 1), family = "serif")
      options(scipen = 999)
      for (i in 1:length(tablePmf2plot))
      {
