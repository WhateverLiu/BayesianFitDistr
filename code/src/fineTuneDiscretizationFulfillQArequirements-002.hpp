    ...
    int maxCore, bool verbose)
{
  ...
  // vec<double*> distables(NsupportSizes * 2);
  // vec<double*> untuneDistables(NsupportSizes);
  // vec<int64_t> distablesSizes(NsupportSizes);
...
//'
//' Transformed Beta discretization
//' 
//' Fine-tuned discretization for fulfilling QA requirements, aka FTDFQA.
//' 
//' @param TrBtable  A numeric matrix of seven rows. Each column contains
...
//' Default to \code{"lr"}.
//' 
//' @param outputProbsInRows  A boolean. \code{TRUE} will produce a PMF table
//' where probabilites are stored in rows. \code{FALSE} will produce one that
//' contains probabilities in columns. Default to \code{FALSE}
//' 
//' @param fineDiscretizationSize  An integer. Support size for raw discretization
//' before tunning. Default to 2000.
//' 
//' @param verbose  A boolean. \code{TRUE} prints checking messages.
//' 
...
//' while \code{FALSE} will increase \eqn{P_0} and downscale the main part probabilities to
//' match the MDR. Default to \code{FALSE}, which is the better choice from
//' experiments.
//' 
...
//' \code{supportSizes}. If \code{outputProbsInRows = FALSE}, every column 
//' of \code{distTable} would be \code{MDR, max, P0, P1, ..., Pmax}. 
//' Otherwise the table will be transposed.
//' 
