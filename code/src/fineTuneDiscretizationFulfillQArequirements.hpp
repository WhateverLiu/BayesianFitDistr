...
struct Ftdfqa
{
    ...
    )
  {
    ...
    // {
    //   Rcout << "r = " << r << "\n";
    //   // for (int i = 0; i < size; ++i) Rcout << val[i] << " ";
    //   // Rcout << "\n";
    //   // for (int i = 0; i < size; ++i) Rcout << P[i] << " ";
    //   // Rcout << "\n";
    //   Rcout << "xmax = " << xmax << "\n";
    //   Rcout << "MDR = " << MDR << "\n";
    //   Rcout << "mean = " << std::inner_product(val, val + size, P, 0.0) << "\n\n";
    // }
    ...
    // Return a range within indicesWanted.
  {
    ...
    // If one of the members is strictly less, then we happy. // Not necessary.
    ...
    int maxCore, bool verbose)
{
  ...
  {
    if (D.cp.maxCore > 1) 
      Rcout << "Some messages will not be printed during multithreading.\n";
    else
    {
      Rcout << "Number of barriers = " << 
