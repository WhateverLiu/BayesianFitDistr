...
if (T)
{
  ...
  if (F)
  {
    ...
    #     unit = "in", res = 120)
    # pdf("../figure/TrBtransitionParam-5-5-2.pdf", width = 9, height = 9 * 0.618)
    ...
    plot(d, xlab = "", ylab = "d", 
         ...
         col = "darkblue")
    lines(x = c(startingPmf, startingPmf), y = c(-1e10, 1e10), lty = 2)
    plot(optRst$fval, xlab = "MDR index", ylab = "", 
         cex.lab = 2, cex.axis = 1.5, type = "l", bty = "L", las = 1, 
         col = "darkblue")
    title(ylab = "Objective function", cex.lab = 1.5, line = 3.6)
    lines(x = c(startingPmf, startingPmf), y = c(-1e10, 1e10), lty = 2)
    legend("bottomright",bty = "n", legend = paste0("Starting MDR index"), 
           lwd = 1, lty = 2, cex = 2)
