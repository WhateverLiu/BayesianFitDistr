...
)
{
  ...
  else originalOdr = 1:length(X)
  
  
  maxNprocess = min(maxNprocess, parallel::detectCores())
  ...
  if (maxNprocess <= 1L)
  {
    ...
    return(lapply(X, function(x) fun(x, commonData))[order(originalOdr)])
  }
  ...
  if (wait)
  {
    ...
    return (  unlist(result[order(originalOdr)], recursive = F)  )
  }
