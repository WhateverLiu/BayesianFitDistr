...
//' 
//' @param supportSize  An integer, e.g. 64 or 42.
//' 
...
//' while \code{FALSE} will increase \eqn{P_0} and decrease the main part to
...
//' \itemize{
//' \item\code{$distTable}: Final distribution table. If 
//' \code{outputProbsInRows = FALSE}, every column of \code{distTable} would be 
//' \code{MDR, max, P0, P1, ..., Pmax}. Otherwise it is the transpose.
//' 
//' \item\code{distTableBeforeTune}: The distribution table before tuning for
